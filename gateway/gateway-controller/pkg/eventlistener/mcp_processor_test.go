...
func testMCPStoredConfig(uuid, handle, displayName, version string, status models.ConfigStatus, policies []api.Policy) *models.StoredConfig {
	now := time.Now()
...
func TestHandleEvent_MCPDelete_RemovesLocalStateAndPolicy(t *testing.T) {
	store := storage.NewConfigStore()
	cfg := testMCPStoredConfig("mcp-delete-id", "test-mcp", "Test MCP", "v1.0.0", models.StatusPending, nil)
